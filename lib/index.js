...
var jsHelper = require("./js-helper");
...
AutoSequelize.prototype.build = function(callback) {
  ...
  function processTables(__tables) {
    ...
    async.each(
      tables,
      compose([mapForeignKeys, mapIndexes, mapTable]),
      callback
    );
  }
  ...
  function mapIndexes(table, fn) {
    ...
    var sql = self.dialect.getIndexesQuery(
      table,
      self.sequelize.config.database
    );
...
AutoSequelize.prototype.run = function(callback) {
  ...
  var tables = [];
  var typescriptFiles = [
    self.options.typescript ? tsHelper.def.getDefinitionFileStart() : "",
    ""
  ];
    ...
    async.each(
      ...
      function(table, _callback) {
        ...
        var tsTableDef = self.options.typescript
          ? "export interface " + tableName + "Attribute {"
          : "";
        ...
        if (!self.options.typescript) {
          ...
          text[table] = tsHelper.model.getModelFileStart(
        ...
        _.each(fields, function(field, i) {
          ...
          if (field === "createtime" || field === "updatetime") {
            ...
            text[table] +=
              ...
              spaces +
              `get() {
            return moment(this.getDataValue('${field}')).format(
              'YYYY-MM-DD HH:mm:ss'
            )
          }` +
              ",\n";
          ...
          if (self.options.typescript)
            tsTableDef += tsHelper.def.getMemberDefinition(
              spaces,
              fieldName,
              tsVal,
              tsAllowNull
        ...
        if (self.indexes[table] && self.indexes[table].length > 0) {
          ...
          _.each(self.indexes[table], function(index) {
            ...
            if (!index.non_unique) {
              text[table] +=
                spaces + spaces + spaces + spaces + "unique: true,\n";
            }
            text[table] +=
              spaces +
              spaces +
          ...
          if (self.options.typescript) {
            text[table] += spaces + spaces + "] as any,\n";
          } else {
            text[table] += spaces + spaces + "],\n";
          }
        }
        ...
        if (self.options.typescript)
          typescriptFiles[0] += tsHelper.def.getTableDefinition(
            tsTableDef,
            tableName
          );
      ...
      function() {
        ...
        if (self.options.typescript) {
          ...
          // add javascript models file
          typescriptFiles[0] = jsHelper.model.generateTableModels(
            _.keys(text),
            self.options.spaces,
            self.options.indentation,
            self.options.camelCase,
...
AutoSequelize.prototype.write = function(
  attributes,
  typescriptFiles,
  callback
) {
  ...
  if (self.options.typescript) {
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      fs.writeFileSync(
        path.join(self.options.directory, "db.d.ts"),
        typescriptFiles[0],
        "utf8"
      );
      fs.writeFileSync(
        path.join(self.options.directory, "db.tables.ts"),
        typescriptFiles[1],
        "utf8"
      );
    }
    ...
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      fs.writeFileSync(
        path.join(self.options.directory, "index.js"),
        typescriptFiles[0],
        "utf8"
      );
    }
  ...
  function createFile(table, _callback) {
    var fileName = self.options.camelCaseForFileName
      ? _.camelCase(table)
      : table;
    fs.writeFile(
      path.resolve(
        path.join(
          ...
          fileName + (self.options.typescript ? ".ts" : ".js")
        )
      ),
      attributes[table],
      _callback
